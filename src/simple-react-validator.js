class SimpleReactValidator {
  ...
  constructor(options = {}) {
    ...
    this.rules = {
      ...
      alpha                : {message: 'The :attribute may only contain letters.',                              rule: val => this.helpers.testRegex(val,/^[\p{Arabic}A-Z]*$/i)},
      alpha_space          : {message: 'The :attribute may only contain letters and spaces.',                   rule: val => this.helpers.testRegex(val,/^[\p{Arabic}A-Z\s]*$/i)},
